...
public final class Sirentide {
    ...
    public static FramesResult renderFramesWithDiagnostics(String dsl,
        ...
        try {
            ...
            if (seqs.size() <= 1) {
                ...
                return new FramesResult(java.util.List.of(base), caveat != null ? caveat
                    : new Diagnostics(Outcome.OK, STAGE_EMIT,
            ...
            Diagnostics caveat = pieDropCaveat(ir);
            return new FramesResult(java.util.List.copyOf(frames), caveat != null ? caveat
                : new Diagnostics(Outcome.OK, STAGE_EMIT, "Rendered successfully.", -1, ""));
        } catch (RuntimeException | StackOverflowError e) {
    ...
    public static RenderResult renderWithDiagnostics(String dsl, com.sirentide.api.MathFragmentRenderer math) {
        ...
        try {
            ...
            // shape: same Outcome.OK, a richer message/detail, no record change.
            Diagnostics caveat = pieDropCaveat(ir);
            if (caveat != null) {
                return new RenderResult(svg, caveat);
            }
            return new RenderResult(svg, new Diagnostics(
    ...
    private static LaidOut layout(Diagram ir, com.sirentide.api.MathFragmentRenderer math) {
        return switch (ir) {
            ...
            // A continuous-score grid: matrix's frame with ramp-interpolated 0..1 cells + a legend.
            case com.sirentide.ir.Heatmap hm -> com.sirentide.layout.HeatmapLayout.layout(hm, math);
            ...
            // A classical knot diagram: one closed curve with transverse crossings (unknot / trefoil /
            // figure8), the under strand broken at each crossing. No `$…$` labels here.
