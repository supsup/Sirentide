...
///
/// FINITE TYPES (the built-in classification, with the shared n ≤
/// {@link DynkinCartan#MAX_RANK} = 200 catalog boundary — {@link #valid()}):
///   * A_n (1 ≤ n ≤ 200): n nodes in a line, all single bonds.
///   * B_n (2 ≤ n ≤ 200): a line whose LAST bond is double, arrow toward the last (short) node.
///   * C_n (2 ≤ n ≤ 200): a line whose last bond is double, arrow AWAY from the last (long) node — B_n and
///     C_n differ ONLY in that arrow direction.
///   * D_n (4 ≤ n ≤ 200): a line of n−2 nodes then a FORK — the (n−2)th node joins two terminal nodes.
...
public record Dynkin(char family, int rank, String textColor) implements Diagram {
    ...
    public boolean valid() {
        return DynkinCartan.isFiniteType(family, rank);
    }
