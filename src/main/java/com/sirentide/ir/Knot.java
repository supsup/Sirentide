...
/// with the UNDER strand BROKEN (a small gap) so the over/under reads unambiguously. Bounded to three
/// classical, small, ALTERNATING knots (over/under alternate along the traversal), each defined by a
/// hard-coded LOOKUP — a deterministic parametric curve plus a crossing table — never a solver:
///
...
///   * `figure8`  — the 4₁ knot (4 crossings): the figure-eight knot; its alternating Gauss code.
///
...
/// normalized knot name (always one of the three above — the parser degrades an unknown/missing type
/// to {@link Empty}, never constructing an inert Knot). `textColor` is threaded for parity with every
/// other type (the knot carries NO `$…$` labels this slice) and defaults to `currentColor`.
public record Knot(String type, String textColor) implements Diagram {
    ...
    /// The 4₁ knot: four crossings, the figure-eight.
