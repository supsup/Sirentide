...
public final class ErDiagramLayout {
    ...
    public static LaidOut layout(ErDiagram er, MathFragmentRenderer math) {
        ...
        // Each table's fan: the SAME label metrics + label column x + baseline emitSelfLoop will use.
    ...
    /// construction, whatever the label's width — and its baseline ALIGNED WITH ITS OWN LOOP'S TOP
    /// HORIZONTAL LEG ({@link #loopLabelBaselines}: optically centred on that line, with a
    /// one-line-slot collision floor for a short table), so each label reads as riding its own loop
    /// the way an edge label rides its edge. The whole fan is then shifted by the table's corridor
    /// `fanShift` (layout's {@link SelfLoopFanShift} solve — the set clears every neighbour-edge
    /// corridor, ordering and pitch preserved), floored below the canvas top edge and clamped
    /// in-canvas (belts: layout reserved this exact column width and grew the bottom with the SAME
    ...
    private static void emitSelfLoop(List<Shape> shapes, Placed table, ErRelation r, String textColor,
        ...
        // every lane line AND every cardinality marker by construction. Y is what associates: this
        // lane's baseline sits ON ITS OWN loop's top horizontal leg ({@link #loopLabelBaselines} —
        // optically centred on that line, with a one-line-slot floor so a short table's clamped
        // attaches degrade into a stack instead of overprinting). The whole fan then rides the table's
        ...
        // ascent floor and canvas clamps are belts: layout reserved this column's width and grew the
        // height with the SAME baseline computation.
        if (r.label() != null && !r.label().isBlank()) {
            String lbl = FONT.ellipsize(r.label(), MAX_LABEL_W, EDGE_LABEL_SIZE);
            double w;
            double asc;
    ...
    /// The label BASELINES of a table's `laneCount` self-loop lanes, indexed by lane — the ONE
    ...
    /// to its own loop, so a reader could not tell which label belonged to which loop). The class
    /// twin carries the identical computation.
    ///
    ...
    /// SHORT table whose attach nudges CLAMP together (all ideals equal) therefore degrades gracefully
    /// into the old evenly-spaced stack instead of overprinting. On every table this layout actually
    /// produces the floor is inert — the sizing pass grows a multi-lane table until the attach step
