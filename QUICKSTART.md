...
> glyphs, and **twenty diagram types** — `pie`, `xychart`, `timeline`, `gantt`, `flowchart`,
> `sequence`, `state`, `quadrant`, `classDiagram`, `erDiagram`, `gitGraph`, `journey`, `mindmap`,
> `sankey`, `mathblock`, `matrix`, `snake`, `tensornetwork`, `young`, and `dynkin`. Also built: **LaTeX math in labels** (the LatteX bridge), **semantic
...
| Mark | Meaning |
|---|---|
| **built** | Works today: the pipeline, the font-metrics oracle, labels-as-paths, all **twenty** diagram types (`pie`, `xychart`, `timeline`, `gantt`, `flowchart`, `sequence`, `state`, `quadrant`, `classDiagram`, `erDiagram`, `gitGraph`, `journey`, `mindmap`, `sankey`, `mathblock`, `matrix`, `snake`, `tensornetwork`, `young`, `dynkin`), semantic anchors (`data-sirentide-role/id/seq`), LatteX-math-in-labels, `alt`/`loop`/`par` sequence frames **and activation bars**, the baked-frame play-through API (`renderFrames`), and the `/docs` ```` ```sirentide ```` fence. |
