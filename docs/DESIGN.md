...
- **M0** — the two contracts + LatteX thin dep + OFL/Apache sans (SHA-pinned in NOTICE, build-time-only via text=paths) + theming-without-`<style>` (inline fills, `currentColor` for light/dark) + bake-time error handling (malformed → inert, never raw HTML) + own-DSL decision + core scaffold (IR, layout/emit split, font-metrics oracle). Emitter alphabet starts minimal: `svg,g,path,rect,line,text-as-paths` + geometry + numeric `fill/stroke/stroke-width/stroke-dasharray`; **NO `marker`/`defs` yet.**
- **M1 — GO/NO-GO** — pie + xychart (arithmetic layout) **plus a minimal linear sequence** (~150 LOC grid arithmetic, no alt/loop/par blocks) carrying a **play-through**, and a math-in-a-label demo. Proves the *spin* (does a baked, semantically-anchored flow you can play feel alive?), not just the seam. ~1–2 weeks.
- **M2** — full sequence (alt/loop/par, self-messages, activations) at its real ~3–6 week size; `marker`/`defs` added value-constrained (same-doc `#id` refs only).
- **M3** — gantt / timeline.
- **M-gate** — flowcharts. Graph auto-layout is dagre-hard (crossing-minimization is NP-hard); **ELK** (a Java layout library) is the reference if we ever do it. Explicit re-decision, not a default.
