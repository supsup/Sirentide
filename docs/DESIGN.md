...
- **Directable / playable flows** *(planned — the thesis frontier)* — step through a flow; reveals in reading order; the active step pulses. A process as a narrated slideshow, not a snapshot. Rides on the semantic anchors (emit complete — every diagram type is anchored).
